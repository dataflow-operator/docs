...
source:
  ...
  kafka:
    ...
    # При true каждое сообщение оборачивается в JSON: {"value": <данные>, "_metadata": {offset, partition, timestamp, key, topic}}
    rawMode: true
...
- **Режим сырой записи (rawMode)**: При включении каждое сообщение оборачивается в JSON с полями `value` (данные) и `_metadata` (offset, partition, timestamp в миллисекундах, key, topic). Удобно для сохранения полного контекста Kafka-сообщения
...
source:
  ...
  trino:
    ...
    # Режим сырой записи (опционально, по умолчанию: false)
    # При true каждая строка оборачивается в JSON: {"value": <данные строки>, "_metadata": {catalog, schema, table, id}}
    rawMode: true
...
- **Метаданные**: Каждое сообщение содержит метаданные:
  ...
  - `table` - название таблицы
- **Режим сырой записи (rawMode)**: При включении каждая строка оборачивается в JSON с полями `value` (данные строки) и `_metadata` (catalog, schema, table, id)
