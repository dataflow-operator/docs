...
Схема при `autoCreateTable: true`: колонка `data` (string) + колонки по ключам metadata из **первого** батча (типы выводятся автоматически: int/long/string/bool). Колонка `_metadata` **не** создаётся. Новые ключи metadata в последующих батчах игнорируются (логируется предупреждение).
