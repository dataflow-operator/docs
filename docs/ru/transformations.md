...
| Filter | Оставляет сообщения по условию (истинность / `==` / `!=`) | 1 сообщение | 0 или 1 сообщение |
...
Тот же, что у [Router](#router):

- **Истинность**: `$.field` — проходит, если поле существует и «истинно» (булево `true`, непустая строка, ненулевое число).
- **Равенство**: `$.field == 'value'` или `$.field == "value"` — проходит, когда значение поля равно заданному (также принимаются литералы без кавычек, например `true` / `5`).
- **Неравенство**: `$.field != 'value'` — проходит, когда поле существует и не равно заданному значению.

Отсутствующее поле не проходит ни проверку истинности, ни сравнение.
...
- **Равенство**: `$.field == 'value'` или `$.field == "value"` — совпадение, когда значение поля равно заданному (также принимаются литералы без кавычек, например `true` / `5`).
- **Неравенство**: `$.field != 'value'` — совпадение, когда поле существует и не равно заданному значению.
