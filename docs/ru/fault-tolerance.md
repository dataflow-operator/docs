...
4. **Trino `queryTimeoutSeconds`**: Выбирайте таймаут с запасом под пиковую нагрузку; слишком малые значения дают ложные падения длинных INSERT.
5. **batchFlushIntervalSeconds**: Меньшие интервалы чаще сбрасывают батч, уменьшая объём данных в зоне риска.
6. **Error sink**: Настройте `spec.errors` для сохранения неудачных сообщений для повторной обработки или анализа.
